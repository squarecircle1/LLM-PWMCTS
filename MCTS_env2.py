  ...
  for actions in action_list:
    for agent, action in actions.items():
      ...
      if 'position' in action:
        coordinates = re.search(r'position[\(\[](.*?)[\)\]]', action).group(1)
...
def remove_agents_and_actions(agent_actions_dict, agents_to_remove, boxes_to_remove):
    # 删除指定的智能体
    ...
    for agent, actions in agent_actions_dict.items():
        agent_actions_dict[agent] = [action for action in actions if not any(box in action for box in boxes_to_remove)]
    agent_actions_dict = {agent: actions for agent, actions in agent_actions_dict.items() if actions}
    agents_only_wait = [agent for agent, actions in agent_actions_dict.items() if len(actions) == 1 and actions[0] == 'WAIT']

    # 如果只有一个wait, 就把可行动作删除
    for agent in agents_only_wait:
        if agent in agent_actions_dict.keys():
            del agent_actions_dict[agent]
...
def are_adjacent(agent1, agent2):
    ...
    # 检查是否在同一行或同一列，并且距离为1
    return (x1 == x2 and abs(y1 - y2) == 1) or (y1 == y2 and abs(x1 - x2) == 1) or (abs(x1 - x2) == 1 and abs(y1 - y2) == 1)
...
def get_conflict_graph(agents_actions):
    ...
    for agent in agents_actions:
        G.add_node(agent)
    box_to_agents = find_agents_for_boxes(agents_actions)
    for box, agents in box_to_agents.items():
        if len(agents)>1:
...
class MCTS_env:
    ...
    def step(self, action):
        ...
        _, next_state, _=action_from_response(self.state, action)
    ...
    def random_action(self, pg_dict):
        ...
        box_exist = []
        for position, items in pg_dict.items():
            x, y = map(float, position.split('_'))
            for item in items:
                if 'box' in item:
                    box_positions[item] = (x, y)
                    box_exist.append((x, y))
        response_dict = {}
        for box, pos in box_positions.items():
            # 随机选择box周围的agent
            surround_agent = position_around(pos[0], pos[1], self.pg_row_num, self.pg_column_num)
            selected_agent = random.choice(surround_agent)
            action_list = []
            if 'target'+box[3:-1] in pg_dict[str(selected_agent[0])+'_'+str(selected_agent[1])]:
                action_list.append('target'+box[3:-1])
                # response_dict[f'Agent[{selected_agent[0]}, {selected_agent[1]}]'] = f'move({box}, target{box[3:-1]})'
                # continue
            for surround_index in position_around(selected_agent[0], selected_agent[1], self.pg_row_num, self.pg_column_num):
                if surround_index not in box_exist:
                    action_list.append(surround_index)
            if action_list:
                selected_action = random.choice(action_list)
                if type(selected_action) == str:
                    response_dict[f'Agent[{selected_agent[0]}, {selected_agent[1]}]'] = f'move({box}, {selected_action})'
                else:
                    response_dict[f'Agent[{selected_agent[0]}, {selected_agent[1]}]'] = f'move({box}, position[{selected_action[0]}, {selected_action[1]}])'
                box_exist.append(selected_action)
                    ...
                    if len(pg_dict_copy[str(corner_x)+'_'+str(corner_y)]) == 1:
                        ...
                        for surround_index in corner_position(i, j):
                            if surround_index != (corner_x, corner_y):
                                action_list.append(f'move({box}, position[{surround_index[0]}, {surround_index[1]}])')
                        if 'target'+box[3:-1] in pg_dict_copy[str(i+0.5)+'_'+str(j+0.5)]:
    ...
    def is_termination(self, pg_dict, assigned_agents, assigned_actions):
        ...
        agents_actions = remove_agents_and_actions(agents_actions, assigned_agents, boxes_to_remove)
    ...
    def get_agents_group_actions_func(self, pg_dict, next_agents, assigned_actions):
        ...
        for action in assigned_actions.values():
            if 'position' in action:
                coordinates = re.search(r'position[\(\[](.*?)[\)\]]', action).group(1)
                next_x, next_y = map(float, coordinates.split(', '))
                assigned_position.append((next_x, next_y))

        agents_actions = {}
        for agent in next_agents:
            action_list = []
            agent_x, agent_y = parse_agent_position(agent)
            for corner_x, corner_y in square_corner_position(agent_x, agent_y):
                if len(pg_dict_copy[str(corner_x)+'_'+str(corner_y)]) == 1:
                    box = pg_dict_copy[str(corner_x)+'_'+str(corner_y)][0]
                    for surround_index in square_corner_position(agent_x, agent_y):
                        if surround_index != (corner_x, corner_y) and len(pg_dict_copy[f'{surround_index[0]}_{surround_index[1]}'])==0 and surround_index not in assigned_position:
                            action_list.append(f'move({box}, position[{surround_index[0]}, {surround_index[1]}])')
                    if 'target'+box[3:-1] in pg_dict_copy[str(agent_x)+'_'+str(agent_y)]:
                        action_list.append(f'move({box}, target{box[3:-1]})')
            if action_list:
                action_list.append('WAIT')
...
class MCTSAgent:
    ...
    def get_guidlines_fun(self, pg_dict):
        ...
        for box, box_pos in box_pos_dict.items():
            ...
            coordinates = re.search(r'position[\(\[](.*?)[\)\]]', ref_action).group(1)
            next_x, next_y = map(float, coordinates.split(', '))
            ref_state = res[0][0].page_content
            # guidelines += f'\nMove the {box} to the position[{next_x}, {next_y}].'
            guidelines += f'\nmove({box}, position[{next_x}, {next_y}])'
    ...
    def select_expand_action_node(self, state_node):
        ...
        if len(state_node.children) <= self.k*state_node.N**self.a:
            agents_groups_score = self.get_agents_groups_score(state_node.agents_groups, state_node.agents_groups_actions, guidelines)
            ...
            if len(best_agents_group_expand_actions) < len(best_agents_group_joint_actions):
                assigned_agents, assigned_agents_actions, expand_action, expand_action_prob= self.env.get_valid_actions(state_node.state, assigned_agents, assigned_agents_actions, best_agents_group, best_agents_group_expand_actions, best_agents_group_joint_actions, best_agents_group_actions, guidelines=guidelines)
        ...
        while not action_node.termination:
            if len(action_node.children) <= self.k*action_node.N**self.a:
                agents_groups_score = self.get_agents_groups_score(action_node.agents_groups, action_node.agents_groups_actions, guidelines)
                ...
                if len(best_agents_group_expand_actions) < len(best_agents_group_joint_actions):
                    assigned_agents, assigned_agents_actions, expand_action, expand_action_prob = self.env.get_valid_actions(state_node.state, action_node.agent, action_node.action, best_agents_group, best_agents_group_expand_actions, best_agents_group_joint_actions, best_agents_group_actions, guidelines=guidelines)
                    termination = self.env.is_termination(state_node.state, assigned_agents, assigned_agents_actions)
