...
def run_exp(Saving_path, pg_row_num, pg_column_num, iteration_num, query_time_limit, eps, cen_decen_framework = 'test', model_name = 'gpt-3', trial = 1):
    ...
    for index_query_times in range(query_time_limit): # The upper limit of calling LLMs
        ...
        try:
          system_error_feedback, pg_dict_returned, collision_check = action_from_response_collision(pg_dict, action)
...
for pg_row_num, pg_column_num in [(4,8)]:
  if pg_row_num == 2 and pg_column_num == 2:
    query_time_limit = 10
  else:
    query_time_limit = 20
  simulation_num = 50

  framework='llm_mcts_pw_noPriority'
